	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishiCollection', this.meishiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.meishiCollectionPhoto ? this.detail.meishiCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishiCollectionYuyue', {
									yonghuId: _this.user.id,
									meishiCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishiCollectionYuyue/list');
							}
						}
					});
				},
