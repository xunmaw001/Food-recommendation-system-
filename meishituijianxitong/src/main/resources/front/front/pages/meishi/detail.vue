		...
		<view class="">
			<view class="price ">
				...
				<view class="pricejiner"
					  :style="detail.meishiNewMoney>1000? 'width: 450rpx;':detail.meishiNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.meishiNewMoney}}</text>
						<text style="font-size: 20rpx;color: #fff;margin-left: 10rpx;">
							<text v-if="detail.meishiOldMoney!=null">原价￥
								<text style="text-decoration:line-through">{{detail.meishiOldMoney}}</text>
							</text>
						</text>
				</view>
			</view>
				   ...
				   <view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								<view class="">
									<view class="">
										<uni-rate style="float: left;;" allow-half readonly :value="totalScore" />
										<text style="color: rgb(255, 202, 62);font-weight: 600;margin-left: -60rpx;">{{totalScore.toFixed(1)}}</text>
									</view>
								</view>
							</view>
						</view>
							...
							<view v-bind:key="index" class="" style="align-items: center;">
								<view class=""
									...
									<view class="" style="font-size: 14px;display: flex;flex-direction: column;color: #8799A3;"
										...
										<!-- 评分 -->
										<view class="">
											<uni-rate size="13" style="float: left;" allow-half readonly
													  :value="item.meishiCommentbackPingfenNumber" />
										</view>
										<!-- 时间 -->
		...
		export default {
			...
			async onShow(options) {
				...
				let data = {
					...
					condition1Value: this.meishiId,
				}
				let queryScoreRes = await this.$api.queryScore(data);
				this.totalScore = queryScoreRes.data.value
			},
			destroyed: function() {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishi', this.meishiId);
				...
				async getmeishiCollection() {
					...
					let res = await this.$api.page(`meishiCollection`, params);
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`meishiCollection`, params);
					if (res.data.list.length == 1) {
						let meishiCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('meishiCollection', JSON.stringify([meishiCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getmeishiCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishiCollection', {
									yonghuId: _this.user.id,
									meishiId: _this.detail.id,
									meishiCollectionTypes:1
								});
