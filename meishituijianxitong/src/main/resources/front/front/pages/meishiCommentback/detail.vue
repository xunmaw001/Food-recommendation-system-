	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.meishiCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('meishiCommentback', this.meishiCommentbackId);
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('meishiCommentbackYuyue', {
									yonghuId: _this.user.id,
									meishiCommentbackId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/meishiCommentbackYuyue/list');
							}
						}
					});
				},
